...
const getLatestActivities = async (req, res) => {
  ...
  // filter the last 5 activities to check its not the latest one. 
   const filteredActs = allActs.activities.slice(allActs.activities.length - 5, allActs.activities.length - 1).filter((act) => {
    return act.id === data_list[data_list.length - 1].id
   })
  ...
  try {
    while (page_num <= 2) {
      ...
      let response = await axios.get(
        ...
        {
          headers: { Authorization: token },
          params: { per_page: 15, page: page_num },
        }
      );
